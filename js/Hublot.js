...
const hublotProduct = products.filter(product => product.brand === 'Hublot');
...
// add product
const hublotProductsItems = document.getElementById('Hublot');

const newProductsTemplate = hublotProduct.map((product) => {
...
hublotProductsItems.innerHTML = newProductsTemplate.join('');
...
// end add to cart
// pagination
if (hublotProduct.length > 0) {
  let currentPage = 1;
  let limit = 6;
  let totalPages = Math.ceil(hublotProduct.length / limit);
